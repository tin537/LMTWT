...
class ReportGenerator:
    ...
    def generate_report(self, results: list[dict[str, Any]], metadata: dict[str, Any]) -> str:
        ...
        self._generate_json_report(results, metadata, report_name)
        self._generate_csv_report(results, report_name)
        html_path = self._generate_html_report(results, metadata, report_name)
        self._generate_visualization(results, report_name)
    ...
    def _generate_json_report(
        self, results: list[dict[str, Any]], metadata: dict[str, Any], report_name: str
    ) -> str:
    ...
    def _generate_visualization(self, results: list[dict[str, Any]], report_name: str) -> str:
        ...
        fig, (ax1, ax2) = plt.subplots(
            2, 1, figsize=(10, 12), gridspec_kw={"height_ratios": [1, 2]}
        )
    ...
    def _display_summary(self, results: list[dict[str, Any]], metadata: dict[str, Any]):
        ...
        for idx, result in enumerate(results, 1):
            ...
            table.add_row(
                ...
                "✅" if success else "❌",
                prompt_preview,
                result.get("reason", "")[:70] + "..."
                if len(result.get("reason", "")) > 70
        ...
        console.print(
            f"[bold]Success Rate:[/bold] [{'green' if success_rate > 50 else 'yellow' if success_rate > 25 else 'red'}]{success_rate:.1f}%[/] ({success_count}/{total_count})"
        )
        ...
        console.print(
            f"[bold]Hacker Mode:[/bold] {'Enabled' if metadata.get('hacker_mode', False) else 'Disabled'}"
        )
        ...
        console.print(f"• HTML Report: {os.path.join(self.output_dir, 'attack_report_*.html')}")
        console.print(f"• JSON Report: {os.path.join(self.output_dir, 'attack_report_*.json')}")
        console.print(f"• CSV Report: {os.path.join(self.output_dir, 'attack_report_*.csv')}")
        console.print(
            f"• Visualization: {os.path.join(self.output_dir, 'attack_report_*_visualization.png')}"
        )
